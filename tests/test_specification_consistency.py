...
class TestSpecificationConsistency(unittest.TestCase):
    ...
    def test_distinct_oc_and_ov_hardware_break_latency_budgets(self):
        """Verify distinct OC (491 ns) and OV (241 ns) latency breakdowns <= 2.0 us."""
        t_shunt = 10e-9
        t_csa_cross = 300e-9
        ...
        t_divider = 10e-9
        t_buffer = 50e-9
        t_trip_ov = t_divider + t_buffer + t_comp_hrtim + t_gate_driver + t_power_stage
        self.assertAlmostEqual(t_trip_ov * 1e9, 241.0, delta=1.0)
        self.assertLess(t_trip_ov, self.T_TOTAL_TRIP_MAX_US * 1e-6)

    def test_itemized_current_trip_error_budget(self):
        """Verify itemized error budget summing to +/-150 mA (+/-3.0%)."""
        err_shunt = 50.0   # mA
        err_gain = 10.0    # mA
        err_csa_vos = 2.5  # mA
        err_dac = 50.0     # mA
        err_comp_vos = 25.0 # mA
        err_temp = 12.5    # mA

        total_err_ma = err_shunt + err_gain + err_csa_vos + err_dac + err_comp_vos + err_temp
        self.assertAlmostEqual(total_err_ma, 150.0, places=1)
        i_trip_min = self.I_HARDWARE_TRIP - (total_err_ma / 1000.0)
        i_trip_max = self.I_HARDWARE_TRIP + (total_err_ma / 1000.0)
    ...
    def test_wcet_execution_schedule(self):
        """Verify 5.0 us WCET schedule (25% CPU load @ 170 MHz) is within 8.0 us budget."""
        t_adc_conv_us = 1.00
        t_dma_irq_us = 0.20
        t_pid_calc_us = 3.60
        t_ccr_reload_us = 0.20

        t_wcet_us = t_adc_conv_us + t_dma_irq_us + t_pid_calc_us + t_ccr_reload_us
        self.assertAlmostEqual(t_wcet_us, 5.00, places=2)
        self.assertLess(t_wcet_us, self.T_ISR_BUDGET_US)
